...
This iteration training can be taken as a stand-alone session, or integrated into a more involved programme with the other sessions. You can find further details on the [TURAS page](), join the [KIND Learning Network](https://forms.office.com/r/WQdd6HSCEW) for social learning support, or contact [Brendan Clarke](mailto: brendan.clarke2@nhs.scot?subject=iteration training) with any queries, comments, or suggestions. You can also find the source code for this training on the [GitHub repository](https://github.com/bclarke-nes/Intermediate-R).

## Introduction

Once again, this training is influenced by the explanations and code from [R4DS](https://r4ds.had.co.nz/iteration.html#for-loops). It's impossible to recommend this book as a resource too strongly, and a quick read of the iteration chapter will be a useful adjunct to this session.

There's a bit more base R in this session than most of the KIND Learning Network training. We'll explain anything new that crops up, and most of the base R code is pretty straightforward. A couple of useful examples that we'll include here are the base R ways of selecting from tibbles. For some tibble called `df`:
...
Can you finish off the 10 green bottles loop above? Bonus points if you can update the final iteration to [fit the lyric "no green bottles"](https://en.wikipedia.org/wiki/Ten_Green_Bottles):
...
However, `conditional_cube()` won't work properly on a vector. Let's try it now, using the useful function `try()`, which attempts to run code, and returns the error message if it doesn't work. That allows us to run dodgy code in Rmarkdown without breaking the knitting process, as would usually happen when we encounter an error:

```{r}
try(conditional_cube(cube_inputs))
```

That `condition has length > 1` in the error message is R's way of telling us that `conditional_cube()` is a *scalar* function that require one item to be supplied to it at a time. While there aren't that many scalar functions in widespread R use now, one major exception is `if()`/`else()`. These are the basis of our `conditional_cube()` misfire, and as conditional execution is a common reason for writing your own functions, unexpectedly encountering a scalar function can be an irritating little problem.

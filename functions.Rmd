...
If you haven't glanced at it before, the book *R for Data Science (R4DS)* comes highly recommended. And we'll follow lots of the advice given in the [R4DS chapter on functions](https://r4ds.had.co.nz/functions.html) in this training session. For example, the authors of that chapter advise readers to consider writing a function whenever you repeat a section of code more than twice. The rationale for this advice is that repetition makes code more complicated. Complicated code is then harder to understand and rework than simple code.
...
Imagine that we've ignored the [R4DS advice](https://r4ds.had.co.nz/functions.html) about copy/pasting, and have repeated the same code a few times, updating both the `labs()` and the `filter()` calls:
...
A practical introduction to indirection can be found [in the *Programming with dplyr vignette*](https://dplyr.tidyverse.org/articles/programming.html). Again, there's a bit of complexity here, and we have training that covers this available in other parts of the KIND Learning Network intermediate R programme - particularly the session on tidyevaluation. 

[The *Advanced R* chapter on quasiquotation](https://adv-r.hadley.nz/quasiquotation.html) is also helpful to understand why this kind of indirection is needed in functions. While the chapter is challenging, it does really help in giving a sense of what's going on behind the scenes. Practically, though, the best way of developing your skills with functions is practice. A little bit of trial and error, and a lot of focused searching for help is an excellent way to learn.

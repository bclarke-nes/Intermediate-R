...
Drafted, needs a good restructure - all the elements are here, but not yet in the right order

----

Writing R as a beginner is largely about learning which functions do which jobs. In this training, we'll learn how to write our own functions. This is important, because turning your code into functions is one useful way of keeping your code comprehensible (to others, and to yourself).
...
Imagine that we've ignored the advice, and have copy/pasted the code a couple of times, updating both the `labs()` and the `filter()` calls:
...
## Simple functions
We've already been playing around with a simple function. Now that we've spent some time thinking about function syntax, let's have a more detailed look at how our function works.
...
```
So when we want to print our range (which goes from `r range_o_matic()[[1]]` to `r as.integer(range_o_matic()[[2]])`) we don't need to put all that dplyr inline, which helps keep your code clean and organised. And we can now quickly print the range of the admissions variable that we have in our data, which is useful enough. 

### Exercise

Let's now tweak `range_o_matic()`. At the moment this gives us the range of the `admissions` variable. Can you tweak the function so that it gives us the range of the `attendances` variable instead? I'll start you off with the existing code.
...
So, we'll conclude with an exercise. Can you build your own function along the lines of `diff_sum`? I'll start you off with some working code:
...
+ start with the bare-bones of the function
+ copy all the code inside the function
+ review the code, so that instead of `number` being hard-coded, we instead use an argument to pass the number to the function
...
Back to `range-o-matic()`. We wanted to adapt this so that we could supply it with any variable in any data set, and have it return the range. Let's have a go at doing that:
...
```
Here, the `$` is the link between the tibble and the column. Not having to explictly specify the data each time is usually a tremendous advantage. When it comes to functions, though, it's a bit more tricky.

<!-- Link to programming with dplyr -->
...
This gets complicated [see the *Advanced R* chapter](https://adv-r.hadley.nz/quasiquotation.html) for many more details. Practice is important here - a little bit of trial and error, and a lot of focused searching for help is an excellent way to learn.
...
```
Okay, interesting enough. But what's much more interesting is that we can be more ambitious than just printing odd or even. Let's see if we can handle our org_code problem more elegantly than just printing an empty graph.
...
my_fun <- function(org_code_arg) {
  ...
  if (nrow(ae_attendances %>% filter(admissions < 5 & breaches >= 400 & org_code == {{org_code_arg}})) == 0) {
    ...
    cat(paste0("No data exists for ", org_code_arg, ".  \n"))
...
<!-- Just a taster, really - ?/save for special tidyverse functions module -->
...
Again, this produces a bit of Rmarkdown:
```{r}
graph_caption()
```
...
As a final thought, this is a really strong combination with `purrr()`, which is covered in another training session. `purrr()` allows you to repeat functions over other objects, like tibbles. To illustrate:
...
walk2(authors$name, authors$org, graph_caption)
```

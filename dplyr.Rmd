...
Before we start, [Rstudio's dplyr cheatsheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-transformation.pdf) is extremely helpful to have at hand while learning new functions.
...
Before we start learning some new dplyr functions, we should point out a feature of the data in the five `NHSRdatasets` package. This data is [tidy](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html). By this, we mean that:
...
+ joins = merging two tibbles together in some way, while broadly preserving the structure of each
  + binding = joins where data are added based on position
  + mutating joins = joins where data are added by values (so matching existing data)
...
To get started with mutating joins in dplyr, we could do much worse than the first part of the man page, which is nice and clear:

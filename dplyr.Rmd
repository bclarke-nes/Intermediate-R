...
3. A brief introduction to tidyselect, where we'll learn some new ways of referencing columns in dplyr. This material is covered in more details in the session on tidy evaluation. We'll also use this section as a refresher on core dplyr functions
...
+ as well as a basic familiarity with using R to manage data

## NHS-R data {.tabset}

Because dplyr provides tools for working with data, we'll need some suitable data to play with. R comes with lots of interesting data sets built-in (and you can explore these using `data()`), in this training we'll use some health data put together by the [NHS-R community](https://nhsrcommunity.com/). You can find additional explanatory materials for these NHS data sets on [CRAN](https://cran.r-project.org/web/packages/NHSRdatasets/index.html), but the short version is that the `NHSRdatasets` package has 5 data sets:
...
Let's look at some data from `NHSRdatasets` package below. If you're following the code, you'll see that there are a couple of helper functions to simplify some of the display work here. We'll start to use these datasets in the next section:
...
The data in the five `NHSRdatasets` package is nicely [tidy](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html). By this, we mean that:

+ Every column is a variable.
+ Every row is an observation.
+ Every cell is a single value.

Tidying data is an art of its own. For this training session, though, you can assume that the data that we'll be working with starts off in this tidy format. 

We'll start with four functions that are useful when you're investigating new data - `slice()`, `glimpse()`, `rename()` and `relocate()`:
...
Like `mutate()`, the general syntax for `rename()` is *new name* = *old name*.
...
```
`rename()` works well with tidyselect operators, which we'll cover briefly during this session. We also have a stand-alone session on [tidy evaluation](https://dplyr.tidyverse.org/articles/programming.html), which covers this territory in more depth.
...
In the tidy evaluation training session, we look across this material in much more depth. There's quite a lot to absorb, so I would definitely recommend using the manual pages (using `??tidyselect`) as a reference for this section.
...
You can combine the `contains()` statement with other `select()` criteria:
...
Something that's confusing to a beginner is that adding a column name to `tally()` performs a weighted count of that column:
...
Both `count()` and `tally()` have `add_` variants:
...
There's also a `c_across()` function to select columns that looks really promising for `rowwise()` work, but bafflingly it is extremely slow here, taking 50x longer than the `mutate()` with a vector of column names in the first code chunk of this section. This is a known issue - particularly for long, narrow, data - https://github.com/tidyverse/dplyr/issues/4953. So this code is switched off and provided here for information only.
...
## Joins and pivots {.tabset}

+ joins = merging two tibbles together in some way, while broadly preserving the structure of each
+ pivots = changing the shape of a tibble (e.g. from wide to narrow) by reworking the columns and the data they contain 

### Joins {.tabset}

To demonstrate how joins work in dplyr, we need to start with some suitable data. We could take take one of the NHSRdatasets, like `ons_mortality`, and split it up by the category_1 column using `group_by()` and then `group_split()`:

```{r}
ons_mortality_list <- ons_mortality %>%
  group_by(category_1) %>%
  group_split()
```

That gives us a list containing `r length(ons_mortality_list)` tibbles. We can access these like accessing any other list item:

```{r}
ons_mortality_list[[2]] %>%
  display()
```

We'll also set up some extremely simple data using `tribble()` to demonstrate how the joins work:
...
To get started with our look at joins, we could do much worse than the first part of the man page, which is nice and clear:
...
`inner_join()` joins the two tibbles by whatever key column you supply using `by=`:
...
We get all the rows that exist in both tibbles.
...
We get all the rows that exist in simple_a, and any that match keys from simple_b. Any 'extra' rows in simple_a are filled in with `NAs`. We've also tweaked the column names in this example, using the `suffix` argument.
